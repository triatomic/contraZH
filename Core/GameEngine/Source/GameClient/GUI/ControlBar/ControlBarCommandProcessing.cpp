...
extern Bool isInBuildVariations(const ThingTemplate* ttWithVariations, const ThingTemplate* b);
	...
	else switch( commandButton->getCommandType() )
	{
		...
		case GUI_COMMAND_EXIT_CONTAINER:
		{
			...
			for (i = 0; i < MAX_COMMANDS_PER_SET; i++)
			{
				...
				if (objWantingExit->getTemplate()->isEquivalentTo(typeEvac) || isInBuildVariations(typeEvac, objWantingExit->getTemplate()))
				{
