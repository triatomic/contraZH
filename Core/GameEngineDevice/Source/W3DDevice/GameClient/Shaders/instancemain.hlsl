...
VsOut main(VsIn input)
{
    ...
    float3 worldNormal = float3(dot(input.World0.xyz, input.Normal), dot(input.World1.xyz, input.Normal),
        dot(input.World2.xyz, input.Normal));
    ...
    float3 color = emissive.rgb + input.Ambient * ambient.rgb;
    for (int i = 0; i < 4; ++i)
    {
        float3 light = LightDiffuse[i].rgb + input.DiffuseOffset * LightDirection[i].w;
