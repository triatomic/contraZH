...
# Every terrain variant again with the seabed's hex tiling, named seabed[lit][bump][noise|noise2][packed|noshadow].pso.
foreach(RTS_SEABED_LIT 0 1)
    foreach(RTS_SEABED_BUMP 0 1)
        foreach(RTS_SEABED_NOISE 0 1 2)
            foreach(RTS_SEABED_SHADOW shadowed packed noshadow)
                set(name "seabed")
                if(RTS_SEABED_LIT)
                ...
                if(RTS_SEABED_NOISE EQUAL 1)
                    string(APPEND name "noise")
                elseif(RTS_SEABED_NOISE EQUAL 2)
                    string(APPEND name "noise2")
                endif()
                set(shadowed 1)
                set(packed 0)
                if(RTS_SEABED_SHADOW STREQUAL "packed")
                    string(APPEND name "packed")
                    set(packed 1)
                elseif(RTS_SEABED_SHADOW STREQUAL "noshadow")
                    string(APPEND name "noshadow")
                    set(shadowed 0)
                endif()
                rts_add_shader("${RTS_SHADER_DIR}/terrainshadow.hlsl" ps_2_a main ${name}.pso NOISE_COUNT=${RTS_SEABED_NOISE}
                    SHADOWED=${shadowed} PACKED=${packed} BUMP=${RTS_SEABED_BUMP} LIGHTS=${RTS_SEABED_LIT} SEABED=1)
            endforeach()
        endforeach()
    endforeach()
endforeach()
